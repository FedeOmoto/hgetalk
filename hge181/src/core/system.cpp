...
bool CALL HGE_Impl::System_Initiate()
{
	...
	if (!RegisterClass(&winclass)) {
		// Only continue if last-error code value is ERROR_CLASS_ALREADY_EXISTS.
		// This hack lets us call this C++ method from Dolphin using finalization.
...
void CALL HGE_Impl::System_SetStateString(hgeStringState state, const char *value)
{
	...
	switch(state)
	{
		case HGE_ICON:			szIcon=value;
								...
								break;
		case HGE_TITLE:			strcpy_s(szWinTitle,sizeof(szWinTitle),value);
								if(pHGE->hwnd) SetWindowText(pHGE->hwnd, szWinTitle);
								break;
		case HGE_INIFILE:		if(value) strcpy_s(szIniFile,sizeof(szIniFile),Resource_MakePath(value));
								else szIniFile[0]=0;
								break;
		case HGE_LOGFILE:		if(value)
								{
									strcpy_s(szLogFile,sizeof(szLogFile),Resource_MakePath(value));
									fopen_s(&hf, szLogFile, "w");
									if(!hf) szLogFile[0]=0;
									else fclose(hf);
								}
								else szLogFile[0]=0;
...
void CALL HGE_Impl::System_Snapshot(const char *filename)
{
	...
	if(!filename)
	{
		...
		sprintf_s(tempname, sizeof(tempname), "shot%03d.bmp", i);
...
HGE_Impl::HGE_Impl()
{
	...
	procExitFunc=0;
	szIcon=0;
	strcpy_s(szWinTitle,sizeof(szWinTitle),"HGE");
	nScreenWidth=800;
	nScreenHeight=600;
	nScreenBPP=32;
	bWindowed=false;
...
{
	System_Log(error);
	strcpy_s(szError,sizeof(szError),error);
}
