...
hgeFont* CALL hgeFontCreate(const char *filename, bool bMipmap=false)
{
	return new hgeFont(filename, bMipmap);
}

void CALL hgeFontDestroy(hgeFont *fnt)
{
	delete fnt;
}
...
hgeFont::hgeFont(const char *szFont, bool bMipmap)
{
	...
	while(pdesc = _get_line(pdesc,linebuf))
	{
		...
		{
			strcpy_s(buf, sizeof(buf), szFont);
			pbuf=strrchr(buf,'\\');
		...
		else if(!strncmp(linebuf, FNTCHARTAG, sizeof(FNTCHARTAG)-1 ))
		{
			...
			sscanf_s(pbuf, " , %d , %d , %d , %d , %d , %d", &x, &y, &w, &h, &a, &c);
...
void hgeFont::printf(float x, float y, int align, const char *format, ...)
{
	...
	_vsnprintf_s(buffer, sizeof(buffer), _TRUNCATE, format, pArg);
