...
public:
	...
	virtual void	CALL	Clamp() { ColorClamp(r); ColorClamp(g); ColorClamp(b); ColorClamp(a); } // 1
	virtual void	CALL	SetHWColor(DWORD col) {	a = (col>>24)/255.0f; r = ((col>>16) & 0xFF)/255.0f; g = ((col>>8) & 0xFF)/255.0f; b = (col & 0xFF)/255.0f;	} // 2
	virtual DWORD	CALL	GetHWColor() const { return (DWORD(a*255.0f)<<24) + (DWORD(r*255.0f)<<16) + (DWORD(g*255.0f)<<8) + DWORD(b*255.0f);	} // 3
};
