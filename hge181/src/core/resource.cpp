...
bool CALL HGE_Impl::Resource_AttachPack(const char *filename, const char *password)
{
	...
	_strupr_s(szName, sizeof(resItem->filename));
	...
	strcpy_s(resItem->filename, sizeof(resItem->filename), szName);
	if(password) strcpy_s(resItem->password, sizeof(resItem->password), password);
...
void* CALL HGE_Impl::Resource_Load(const char *filename, DWORD *size)
{
	...
	strcpy_s(szName,sizeof(szName),filename);
	_strupr_s(szName, sizeof(szName));
	...
	while(resItem)
	{
		...
		while(done==UNZ_OK)
		{
			...
			if(!strcmp(szName,szZipName))
			{
				...
				{
					unzClose(zip);
					sprintf_s(szName, sizeof(szName), res_err, filename);
					_PostError(szName);
					return 0;
				}
...
char* CALL HGE_Impl::Resource_MakePath(const char *filename)
{
	...
	if(!filename)
		strcpy_s(szTmpFilename, sizeof(szTmpFilename), szAppPath);
	else if(filename[0]=='\\' || filename[0]=='/' || filename[1]==':')
		strcpy_s(szTmpFilename, sizeof(szTmpFilename), filename);
	else
	{
		strcpy_s(szTmpFilename, sizeof(szTmpFilename), szAppPath);
		if(filename) strcat_s(szTmpFilename, sizeof(szTmpFilename), filename);
	}
