...
hgeSprite* CALL hgeSpriteCreateFromTexture(HTEXTURE tex, float x, float y, float w, float h)
{
	return new hgeSprite(tex, x, y, w, h);
}

hgeSprite* CALL hgeSpriteCreateFromSprite(const hgeSprite &spr)
{
	return new hgeSprite(spr);
}

void CALL hgeSpriteDestroy(hgeSprite *spr)
{
	delete spr;
}
